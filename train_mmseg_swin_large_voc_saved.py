"""
使用 mmsegmentation 的 Swin-Large 模型训练完整的 VOC 数据集（21类）
结合 mmsegmentation 的模型架构和完整的 VOC 数据加载
"""
...
class RandomResizedCrop:
    """随机缩放裁剪（与 WILSON 的 RandomResizedCrop 一致）"""
    def __init__(self, size, scale=(0.5, 2.0), ratio=(3./4., 4./3.)):
    ...
    def __call__(self, img, target):
        ...
        # 使用 resized_crop：先裁剪再调整大小
        img = TF.resized_crop(img, i, j, h, w, self.size, TF.InterpolationMode.BILINEAR)
        target = TF.resized_crop(target, i, j, h, w, self.size, TF.InterpolationMode.NEAREST)
        return img, target


class VOCTransform:
    """VOC 数据集转换，适配 mmsegmentation 格式（与 step0_saved.py 一致）"""
    def __init__(self, crop_size=512, train=True):
        ...
        # 训练时的随机缩放裁剪（与 step0_saved.py 一致）
        if train:
            self.random_resized_crop = RandomResizedCrop(crop_size, scale=(0.5, 2.0))
    ...
    def __call__(self, img, target):
        ...
        if self.train:
            # 训练时：随机缩放裁剪 + 随机水平翻转
            img, target = self.random_resized_crop(img, target)
...
class TrainingConfig:
    ...
    def __init__(self):
        ...
        self.eval_interval_iters = 1000  # 每多少次迭代验证一次（迭代级别的验证，与mmsegmentation一致）
        ...
        self.output_dir = os.path.join(
            current_dir, 
            'outputs', 
            f'mmseg_swin_{self.model_size}_voc_21class'
        )
...
def main(config=None):
    """主训练函数"""
    print("=" * 70)
    print("mmsegmentation Swin-Large 训练 - VOC 完整数据集（21类）")
    print("=" * 70)
    ...
    try:
        # 创建数据转换
        train_transform = VOCTransform(crop_size=config.crop_size, train=True)
        val_transform = VOCTransform(crop_size=config.crop_size_val, train=False)
        ...
        logger.info(f"  测试集: {len(val_dataset)} 样本")
    ...
    # ==================== 创建评估指标对象 ====================
    logger.info("\n创建评估指标对象...")
    val_metrics = StreamSegMetrics(n_classes=config.n_classes)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='训练 mmsegmentation Swin-Large 模型 (VOC 完整数据集 21类)')
